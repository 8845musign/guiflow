...
interface UiflowApi {
  update: (inputFileName: string, code: string, format: string) => Promise<Buffer>;
...
api.update = function(inputFileName: string, code: string, format: string): Promise<Buffer> {
    const f = flumine(function(d: any, ok: (value: Buffer) => void, ng: (error: Error) => void) {
        const buff: Buffer[] = [];
        // through2.objを使用するか、through2.defaultを使用する
        const output = through2.obj(function(chunk: Buffer, enc: string, cb: () => void) {
            const svg = chunk;
            buff.push(svg);
            cb();
        });
        ...
        const stream = uiflow.buildWithCode(
            inputFileName, code, format, function(error: Error) {
